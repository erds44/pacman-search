    ...
    while len(openset) > 0:
        ...
        if problem.isGoalState(state):
            ...
            while state != problem.getStartState():
                actions.append(parents[state][1]) # action is the 2nd item in tuple
            ...
            for (next_state, action, cost) in problem.getSuccessors(state):
                if not(next_state in closedset):
                    ...
                    particalset.insert(0, next_state)
                    closedset.append(next_state)
            openset.extend(particalset)
...
def uniformCostSearch(problem):
    ...
    parents = {}
    while len(openset) > 0:
        state = heappop(openset)
        closedset.append(state[1])
        if problem.isGoalState(state[1]):
            print("Found goal!")
            actions = []
            while state[1] != problem.getStartState():
                actions.append(parents[state[1]][1]) # action is the 2nd item in tuple
                state = (0, parents[state[1]][0])
            print(actions)
            actions.reverse()
            return actions
        else:
            for (next_state, action, cost) in problem.getSuccessors(state[1]):
                if not(next_state in closedset):
                    parents[next_state] =(state[1], action)
                    heappush(openset, (state[0] + cost, next_state))
                    closedset.append(next_state)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    closedset = []
    openset = []
    heappush(openset, (heuristic(problem.getStartState(), problem), problem.getStartState()))
    parents = {}
    while len(openset) > 0:
        state = heappop(openset)
        closedset.append(state[1])
        if problem.isGoalState(state[1]):
            ...
            for (next_state, action, cost) in problem.getSuccessors(state[1]):
                if not(next_state in closedset):
                    parents[next_state] =(state[1], action)
                    heappush(openset, (state[0] + cost + heuristic(next_state, problem), next_state))
                    closedset.append(next_state)
    util.raiseNotDefined()
